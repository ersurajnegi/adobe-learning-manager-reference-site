...
//training actions for “active” learner enrollment */
const OFFLINE_DOWNLOAD_ENROLLMENT_STATES = new Set(['ENROLLED', 'STARTED', 'COMPLETED']);

/**
 * Instance ids to include in offline download (multi-enrollment: all active instance enrollments).
 * Falls back to primary / caller instance when instance-level enrollment is not on the payload.
 */
export const getInstanceIdsForOfflineDownload = (
  ...
  if (uniqueDownloadInstanceIds.length > 0) {
    return uniqueDownloadInstanceIds;
  }
  const primaryInstanceId =
    lo.enrollment?.loInstance?.id || (fallbackInstanceId ? String(fallbackInstanceId) : '');
  return primaryInstanceId ? [primaryInstanceId] : [];
};
