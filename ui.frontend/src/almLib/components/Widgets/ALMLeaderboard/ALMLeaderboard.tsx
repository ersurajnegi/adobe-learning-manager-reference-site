...
import { getALMAccount, getALMConfig, getALMObject, getWidgetConfig } from '../../../utils/global';
...
import { ALMNoAccessContainer } from '../../CustomPages/ALMNoAccessContainer';
...
const ALMLeaderboard = (props: any) => {
  ...
  const { getCompetitors, fetchingData } = useLeaderboard();
  ...
  const isMobileApp = getALMConfig().learnerMobileApp;
  const showExploreButton = !(
    isMobileApp ||
    isLeaderboardLinkDisabled() ||
  ...
  const shouldAllowNavigation = () => {
    return !isLeaderboardLinkDisabled() && !disableLinks;
  };

  function isLeaderboardLinkDisabled() {
    // Check widget attributes first for early return
    ...
    return (
      <>
        <div className={styles.beginnerState}>
          ...
          {renderProgressBar()}
          <div>{level.pointsAwayFromNextLevelText}</div>
        </div>
      </>
    );
  };
    ...
    >
      ...
      <div style={{ width: `${containerWidth}px` }}>
        <div className={styles.headerContainer}>
          ...
          {showExploreButton && (
            <button
              className={`${styles.exploreButton} ${allCompetitorsHaveZeroPoints ? '' : styles.explorePosition}`}
              onClick={handleExploreClick}
