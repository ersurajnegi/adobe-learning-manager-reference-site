...
export async function SendLinkEvent(refLink_lxpv: string | null) {
  if (refLink_lxpv) {
    ...
    } else if (containsSubstr(HOME_PAGE_PATH, linkType)) {
      config.navigateToHomePage();
    } else if (containsSubstr(PROFILE_PAGE_PATH, linkType)) {
      config?.navigateToProfilePage();
    }
  }
}
export function SendMessageToParent(msg: any, target: string) {
  const wnd = getWindowObject();

  // Messages that need direct parent window handling (bypass React app)
  const MESSAGES_FOR_PARENT = ['KEYBOARD_SHORTCUTS'];

  // Determine target window based on message type
  let targetWindow = wnd;

  if (wnd.parent !== wnd) {
    // We're in an iframe
    if (MESSAGES_FOR_PARENT.includes(msg.type)) {
      // Send directly to parent (e.g., for keyboard shortcuts like Alt+1)
      targetWindow = wnd.parent;
    } else {
      // Send to self so React app can handle it first
      // React app (App.tsx) will then forward to parent if needed
      targetWindow = wnd;
    }
  }

  console.log('sending message', msg, 'to', targetWindow === wnd ? 'self' : 'parent');
...
const CATALOG_PAGE_LINK_WITH_BOOKMARKS = {
  refLink_lxpv: 'primelink:catalogPageLink',
  desktopLink_lxpv: LEARNER_PREFIX + '#/catalog/index?bookmarks=true',
};
export function GetCatalogPageLinkWithBookmarks() {
  return addToPrimeLinksMap(CATALOG_PAGE_LINK_WITH_BOOKMARKS);
}
...
const USER_PROFILE_PAGE_LINK = {
  refLink_lxpv: 'primelink:userProfilePageLink',
  desktopLink_lxpv: LEARNER_PREFIX + '#/social/userProfile/${userId}',
};
export function GetUserProfilePageLink() {
  return addToPrimeLinksMap(USER_PROFILE_PAGE_LINK);
}

const VIRTUAL_COACH_PAGE_LINK = {
  refLink_lxpv: 'primelink:virtualCoachPageLink',
  desktopLink_lxpv:
    LEARNER_PREFIX + '#/catalog/index?virtualCoachSelected=true&selectedSortOption=-date',
};

export function GetVirtualCoachPageLink() {
  return addToPrimeLinksMap(VIRTUAL_COACH_PAGE_LINK);
}
