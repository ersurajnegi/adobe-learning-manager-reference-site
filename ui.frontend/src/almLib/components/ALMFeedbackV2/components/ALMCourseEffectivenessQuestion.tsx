...
interface CourseEffectivenessQuestionProps extends QuestionRendererProps {
  question: CourseEffectivenessQuestion;
  questionText: string;
}
...
export function ALMCourseEffectivenessQuestionComponent({
  question,
  questionText,
  onAnswerChange,
  locale,
  disabled,
  order,
}: CourseEffectivenessQuestionProps) {
  const isMobile = useDeviceTypeContext().isMobile;
  ...
  return (
    <>
      <label id={question.id} lang={locale} className={styles.feedback_mandatory}>
        {questionText}
      </label>
      ...
      <span id={`${question.id}-info`} className={styles.aria_hidden}>
        {GetTranslationsReplaced('alm.feedback.text.courseEffectiveness.description', {
          min: MIN,
          max: MAX,
        })}
      </span>
      <Flex direction={'column'}>
        <Flex width={'100%'}>
          <RadioGroup
            ...
            errorMessage={getErrorMessagehandler(QuestionType.COURSE_EFFECTIVENESS)}
          >
