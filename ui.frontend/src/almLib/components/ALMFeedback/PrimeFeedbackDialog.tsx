...
import { emitL1FeedbackClosed } from './FeedbackUtil';
...
import { GetPrimeEmitEventLinks, getWidgetConfig } from '../../utils/global';
import PrimeFeedbackForm from './PrimeFeedbackForm';
import CrossIcon from '../../assets/images/close_gray_normal.png';
import { useAccount, useNotifications } from '../../hooks';
import { getUserId } from '../../utils/widgets/utils';
import { Question } from '../../models/FeedbackModel';
import { FormRenderer } from '../ALMFeedbackV2';
import { getPrimeFeedbackAnswer, mapQuestionType } from './FeedbackUtil';
...
const PrimeFeedbackDialog: React.FC<{
  ...
  const { updateNotification: updateNotificationAPI } = useNotifications();
  ...
  const isParentCourse = parentLo.loType === COURSE;
  ...
  const isMobileBreakpoint = useDeviceTypeContext().isMobile;
  const connectedCallback = () => {
    const trainingEnrollment = (isParentCourse ? trainingInstance : training).enrollment;
  ...
  const updateNotificationFunction = async () => {
    // dispatch(updateNotification(userNotification.id, { actionTaken: true }));
    const userId = await getUserId();
    userId && updateNotificationAPI(userId, userNotification.id, userNotificationBody);
  };
  ...
  const getFeedbackSubmissionDetails = () => {
    ...
    setShowError(false);
    return getFeedbackSubmissionPayload(answeredQuestions, score);
  };

  const getFeedbackSubmissionPayload = (
    answeredQuestions: PrimeFeedbackQuestion[],
    score: number
  ) => {
  ...
  const getFeedbackFormSubmissionAnswers = (userAnswers: Record<string, any>) => {
    const answeredQuestions: PrimeFeedbackQuestion[] = getPrimeFeedbackAnswer(
      userAnswers,
      state.questions,
      locale
    );

    const courseEffectivenessQuestion = state.questions.find(
      question => question.questionType === SCALE_TEN
    );
    const score = courseEffectivenessQuestion
      ? parseInt(userAnswers[courseEffectivenessQuestion?.questionId]) * 10
      : 0;
    return getFeedbackSubmissionPayload(answeredQuestions, score);
  };

  const handleSubmitButtonV2 = async (form: Record<string, any>) => {
    const feedbackBody = getFeedbackFormSubmissionAnswers(form);
    if (!feedbackBody) {
      return;
    }
    try {
      await submitL1Feedback(enrollment, feedbackBody);
      setFeedbackSubmittedSuccessfully(true);
      updateNotificationFunction();
      setTimeout(() => {
        closeFeedbackPopUp(parentLo.id, userNotification.id);
        emitL1FeedbackClosed([userNotification.id]);
      }, feedbackSubmitTimer);
    } catch (error: any) {
  ...
  const handleSubmitButton = async (formAnswers: Record<string, any> | undefined) => {
  ...
  const closeFeedbackDialog = () => {
    console.log('Feedback close inside dialog component');
    closeFeedbackPopUp(parentLo.id, userNotification.id);
    emitL1FeedbackClosed([]);
  };
  ...
  return (
    <div tabIndex={-1} className={styles.feedback_overlay}>
      <div
        className={styles.feedback_dialog}
        style={account.newFeedbackFormEnabled && !isMobileBreakpoint ? { width: '780px' } : {}}
        role="dialog"
        aria-modal="true"
      >
        {getHeaderDetails()}
        <div className={styles.feedback_body}>
          {account.newFeedbackFormEnabled ? (
            <FormRenderer
              feedbackForm={{
                questions: state.questions
                  .map(question => mapQuestionType(question))
                  .filter((q): q is Question => q !== null),
                locales: [locale],
                defaultLocale: locale,
              }}
              locale={locale}
              onFormSubmit={handleSubmitButtonV2}
