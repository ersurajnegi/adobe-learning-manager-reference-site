...
const socialSearchResults: Reducer<SocialSearchResultList, AnyAction> = combineReducers({
  searchTerm,
  searchBoards,
  searchNext,
  searchPaginating,
...
const contentType: Reducer<'VIDEO' | 'URL' | 'IMAGE' | 'TEXT' | 'FILE' | 'OTHER', AnyAction> = (
...
const data: Reducer<string, AnyAction> = (state: string | null | undefined, action: AnyAction) => {
  switch (action.type) {
    case SET_CONTENT_TYPE:
      return action.data;
    case REMOVE_CONTENT:
