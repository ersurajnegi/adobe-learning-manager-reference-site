...
import { EXTERNAL_SKILL_IFRAME_ID } from '../../../utils/constants';
const TWEEN = require('@tweenjs/tween.js').default;
import Close from '@spectrum-icons/workflow/Close';
import ReactDOMServer from 'react-dom/server';

const ExternalSkillGraphComponent = React.forwardRef((props: any, ref) => {
  const currentDocument = (document.querySelector(`#${EXTERNAL_SKILL_IFRAME_ID}`) as any)
  ...
  const hideSkillsGraph = getALMConfig()?.learnerMobileApp;
  const graphVisibilityClass = hideSkillsGraph ? styles.displayNone : '';
  ...
  useEffect(() => {
    if (renderer) {
      ...
      currentDocument.addEventListener('mouseup', handleClickOutsideTypeahead);
      window.addEventListener('blur', function () {
        if (currentDocument.activeElement != currentDocument.getElementById('iframe')) {
          hideTypeahead();
        }
      });
      ...
      let skillsArea = currentDocument.querySelector(`.${styles.selectedSkillsArea}`);
      skillsArea?.addEventListener('click', handleClickOnSkillsArea);
      skillsArea?.addEventListener('mousedown', (event: any) => event.preventDefault());
    }
    const handleResize = () => {
      const parentWidth = getWidth();
  ...
  function addListenerForCanvasRotation() {
    let element = currentDocument.querySelector('canvas');
  ...
  function addListenerForNodes() {
    let canvasElement = currentDocument.querySelector('canvas');
  ...
  function createSkillBox(node: any) {
    ...
    let element = currentDocument.querySelector(`.${styles.selectedSkillsArea}`);
    let skillBox = currentDocument.createElement('span');
    skillBox.id = node.id;
    ...
    let closeButton = currentDocument.createElement('button');
    closeButton.className = styles.closeButton;
    closeButton.id = 'remove-' + node.id;
    closeButton.setAttribute('automationid', `remove-${node.name}`);
    closeButton.innerHTML = ReactDOMServer.renderToStaticMarkup(<Close />);
    skillBox.appendChild(currentDocument.createTextNode(truncatedName));
    skillBox.appendChild(closeButton);
    element?.appendChild(skillBox);
  }

  function removeSkillBox(nodeId: string) {
    let skillBox = currentDocument.getElementById(nodeId);
  ...
  function updateSelectedSkillCount() {
    let selectedCountElement = currentDocument.querySelector(`.${styles.selectedCount}`);
    if (isNotNullAndUndefined(selectedCountElement)) selectedCountElement?.remove();

    if (selectedNodes.length > 0) {
      let selectedSkillsHeadingElement = currentDocument.querySelector(
        `.${styles.selectedSkillsHeading}`
      );
      selectedCountElement = currentDocument.createElement('span');
  ...
  function showResetGraphIcon() {
    if (userExploreWorkflow === false) {
      ...
      let resetButton = currentDocument.createElement('button');
  ...
  function handleClickOnSkillsArea(event: any) {
    if (!event.target.closest('button') && !event.target.id) {
      return;
    }
    const nodeId =
      event.target.tagName === 'SPAN' ? event.target.id : event.target.closest('button').id;
  ...
  function showNoInterestSelectedMessage() {
    let emptyMessageElement = currentDocument.querySelector(`.${styles.emptyMessage}`);
    if (isNullOrUndefined(emptyMessageElement)) {
      let selectedSkillsArea = currentDocument.querySelector(`.${styles.selectedSkillsArea}`);
      emptyMessageElement = currentDocument.createElement('span');
  ...
  function showSearchLoader() {
    let typeaheadElement = currentDocument.getElementById('typeahead');
    let searchLoaderElement = currentDocument.createElement('img');
  ...
  function hideSearchLoader() {
    let searchLoaderElement = currentDocument.querySelector(`.${styles.searchLoader}`);
  ...
  function handleKeydown(event: any) {
    ...
    if (key === 38) {
      //arrowup
      let suggestionElements = currentDocument.querySelectorAll(`.${styles.suggestion}`);
      if (currentFocus != -1) {
  ...
  function showTypeahead() {
    let typeaheadElement = currentDocument.createElement('div');
  ...
  function addNoResultMessage() {
    let typeaheadElement = currentDocument.getElementById('typeahead') as HTMLElement;
    let noResultElement = currentDocument.createElement('div');
  ...
  function removeNoResultMessage() {
    let noResultElement = currentDocument.querySelector(`.${styles.noSuggestion}`);
  ...
  function addSuggestion(id: string, name: string) {
    let typeaheadElement = currentDocument.getElementById('typeahead') as HTMLElement;
    let suggestionElement = currentDocument.createElement('div');
  ...
  function selectNode(nodeId: string) {
    if (isNotNullAndUndefined(nodeId)) {
      ...
      if (isNullOrUndefined(selectedNode)) {
        ...
        if (selectedNodes.indexOf(parseInt(nodeId)) <= -1) {
  ...
  async function search() {
    ...
    let suggestionElement = currentDocument.querySelector(`.${styles.suggestion}`);
  ...
  function showLoader(show: boolean = true) {
    if (loaderElement.current && !hideSkillsGraph) {
      (loaderElement.current as any).className =
        `${styles.loader}${show ? ` ${styles.visible}` : ''}`;
    }
  }

  function hideResetGraphIcon() {
    let resetGraphButton = currentDocument.querySelector(`.${styles.resetGraph}`);
  ...
  function clearGraph() {
    ...
    currentDocument.getElementById('3dgraph')?.remove();
    hideResetGraphIcon();
  ...
  async function fetchGraphData(allAccounts: boolean, firstLoad: boolean) {
    ...
    if (firstLoad && isNotNullAndUndefined(props.selectedExternalInterest)) {
      for (let nodeId of props.selectedExternalInterest) {
        let node = getNode(nodeId);
        if (isNullOrUndefined(node)) {
          let result = (await RestAdapter.ajax({
    ...
    <>
      <div className={styles.frame}>
        <div className={styles.topOptions}>
          ...
          <span className={`${styles.graphHeading} ${graphVisibilityClass}`}>
            {GetTranslation('alm.skillMapHeading', true)}
          </span>
        </div>
        ...
        <div
          id="graph-area"
          ref={GRAPH_AREA}
          className={`${styles.graphArea} ${graphVisibilityClass}`}
