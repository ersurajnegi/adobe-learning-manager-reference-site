...
import { WidgetMap } from '../../models/CustomPages';
import {
  GetAllCatalogGroupFilterLink,
  GetCatalogPageLink,
  GetCatalogPageLinkWithBookmarks,
  GetMyLearningPageLink,
  GetSkillsPageLink,
  GetVirtualCoachPageLink,
  SendLinkEvent,
...
export function getHeading(
  ...
  switch (widget.widgetRef) {
    ...
    case WidgetType.BOOKMARKS:
      ...
      link = `${GetCatalogPageLinkWithBookmarks()}`;
    ...
    case WidgetType.RECOMMENDATIONS_STRIP:
      ...
      if (account.recommendationAccountType === CPENEW && !account.prlCriteria?.enabled) {
        seeAllLink = '';
        const isMobileWithoutSkillsPage =
          getWidgetConfig()?.isMobile && !getALMConfig()?.learnerMobileApp;
        widget.attributes?.recommendationConfig.stripType === 'SUPER_RELEVANT_STRIP' &&
        !isMobileWithoutSkillsPage &&
    ...
    case WidgetType.CATALOG_BROWSER:
      ...
      break;
    case WidgetType.VIRTUAL_COACH:
      name = GetTranslation('alm.strip.virtualcoach.title', true);
      description = GetTranslation('alm.strip.virtualcoach.description', true);
      link = GetVirtualCoachPageLink();
...
export function configureWidgetsForLayout(layoutConfig: any, user: PrimeUser | null) {
  ...
  return layoutConfig.map((widgets: any[]) => {
    return widgets.filter((widget: any) => {
      ...
      if (widget.widgetRef === WIDGET_REF_MAP_NAME.LEADERBOARD) {
        return user?.gamificationEnabled;
      }
...
export const handleKeyDownEvent = (event: any, link?: string) => {
  // Only intercept activation keys; allow Tab/Shift+Tab and others to bubble
...
export const setIsCustomPage = (isCustomPage: boolean) => {
  _isCustomPage = isCustomPage;
};

export const getIsCustomPage = () => {
  return _isCustomPage;
};
...
const isWidgetValid = (widgetId: string, widgets: WidgetMap): boolean => {
  if (!widgetId || !widgets) {
    return false;
  }
  return Object.values(widgets).some(widget => widget.id === widgetId);
};
const dispatchALMConfigEvent = async (widgetId: string): Promise<void> => {
  const userResponse = await getALMUser();
  const userId = userResponse?.user?.id;
  const accountId = userResponse?.user?.account?.id;
  const { nativeExtensionToken, primeApiURL, locale } = getALMConfig();
  const configEvent = new CustomEvent(PrimeEvent.ALM_EB_SENT_CONFIG, {
    detail: {
      ALM_WidgetId: widgetId,
      ALM_token: nativeExtensionToken,
      ALM_primeApiUrl: primeApiURL,
      ALM_locale: locale,
      ALM_userId: userId,
      ALM_accountId: accountId,
    },
  });
  document.dispatchEvent(configEvent);
};
export const setupALMConfigEventListener = (widgets: WidgetMap): (() => void) => {
  const handleGetConfigRequest = (event: Event) => {
    const customEvent = event as CustomEvent;
    const widgetId = customEvent.detail?.widgetId;

    if (!isWidgetValid(widgetId, widgets)) {
      return;
    }
    dispatchALMConfigEvent(widgetId);
  };

  document.addEventListener(PrimeEvent.ALM_EB_GET_CONFIG, handleGetConfigRequest as EventListener);

  return () => {
    document.removeEventListener(
      PrimeEvent.ALM_EB_GET_CONFIG,
      handleGetConfigRequest as EventListener
    );
  };
};
...
const navigateToPageHandler = async (
  ...
  const navigationMap: Record<string, () => void> = {
    MYLEARNING: () => almObject.navigateToMyLearningPage(),
    CATALOG: () => almObject.navigateToCatalogPage(),
    SKILLS: () => almObject.navigateToSkillsPage(),
    HOME: () => almObject.navigateToHomePage(),
    CUSTOM_PAGE: () => {
      if (!pageId) {
        console.log('Navigation failed: pageId is required for CUSTOM_PAGE');
        return;
      }
      almObject.navigateToCustomPage(pageId);
    },
    TRAINING_OVERVIEW_PAGE: () => {
      if (!trainingId || !instanceId) {
        console.log('Navigation failed: Id is required for TRAINING_OVERVIEW_PAGE');
        return;
      }
      almObject.navigateToTrainingOverviewPage(trainingId, instanceId);
    },
    INSTANCE_PAGE: () => {
      if (!trainingId) {
        console.log('Navigation failed: trainingId is required for INSTANCE_PAGE');
        return;
      }
      almObject.navigateToInstancePage(trainingId);
    },
  };

  const navigateAction = navigationMap[navigateToPage];
  if (navigateAction) {
    navigateAction();
  }
};

export const setupHTMLWidgetNavigationListener = (widgets: WidgetMap): (() => void) => {
  const handleNavigationRequest = (event: Event) => {
    const customEvent = event as CustomEvent;
    const { widgetId, navigateToPage, pageId, trainingId, instanceId } = customEvent.detail || {};

    if (!isWidgetValid(widgetId, widgets)) {
      return;
    }

    navigateToPageHandler(navigateToPage, pageId, trainingId, instanceId);
  };

  document.addEventListener(PrimeEvent.ALM_EB_NAVIGATE, handleNavigationRequest as EventListener);

  return () => {
    document.removeEventListener(
      PrimeEvent.ALM_EB_NAVIGATE,
      handleNavigationRequest as EventListener
    );
  };
};

const extractPaginationFromNextUrl = (
  nextUrl: string,
  currentCursorBased: boolean | undefined
): {
  cursorBased: boolean;
  cursor: string | null;
  offset: string | null;
  ...
  try {
    ...
    const cursorBased = currentCursorBased !== undefined ? currentCursorBased : !!cursor;

    return {
      cursorBased,
      cursor: cursorBased ? cursor : null,
      offset: !cursorBased ? offset : null,
    };
  } catch (error) {
    return null;
  }
...
export const calculatePaginationState = (
  ...
  totalFetched: number,
  pageLimit: number,
  maxItemToFetch: number | undefined,
  currentCursor: string | null,
  currentOffset: string | null,
  cursorBased: boolean | undefined
) => {
  const tempTotalFetched = totalFetched + pageLimit;
  let tempFetchedAll = false;
  let tempCurrentCursor = currentCursor;
  let tempCurrentOffset = currentOffset;
  let tempCursorBased = cursorBased;

  if (!next || (maxItemToFetch && tempTotalFetched >= maxItemToFetch)) {
    tempFetchedAll = true;
  } else {
    const paginationInfo = extractPaginationFromNextUrl(next, cursorBased);
    if (paginationInfo) {
      tempCursorBased = paginationInfo.cursorBased;
      tempCurrentCursor = paginationInfo.cursor;
      tempCurrentOffset = paginationInfo.offset;
    }
  }
