...
const PrimeTrainingPage = (props: any) => {
  ...
  // Prefer props, fallback to params
  let { trainingId, trainingInstanceId } = props;

  if (!trainingId || !trainingInstanceId) {
  ...
  const [showAlternateCompletionDetails, setShowAlternateCompletionDetails] = useState(false);
  ...
  const trainingTypeLabel = useMemo(
    () => getTrainingTypeLabel(training?.loType),
    [training?.loType]
  );
  const alternateTrainingTypeLabel = useMemo(
    () => getTrainingTypeLabel(alternateLo?.loType),
    [alternateLo?.loType]
  );

  const alternateLoDescriptionContent = useMemo(() => {
    const descriptionText = alternateLo
      ? GetTranslationsReplaced(
          'text.alternateCompletionDescription',
          { trainingType: trainingTypeLabel, alternateTrainingType: alternateTrainingTypeLabel },
          true
        )
      : GetTranslationsReplaced(
          'text.alternateCompletionDescriptionWithoutLink',
    ...
    return (
      <>
        <span className={styles.alternateLoDescription}>{descriptionText}</span>
        {alternateLo && (
          <a
            href={getTrainingLink(alternateLo.id, account.id)}
            className={styles.alternateLoLink}
            onClick={e => {
              e.preventDefault();
              getALMObject().navigateToTrainingOverviewPage(alternateLo.id);
            }}
          >
            {alternateLoName}
          </a>
  ...
  }, [alternateLo, alternateLoName, trainingTypeLabel, alternateTrainingTypeLabel, account.id]);

  const expandedInfoText = useMemo(() => {
    const alternateTrainingType =
      alternateTrainingTypeLabel || GetTranslation('text.courseOrPath', true);

    return GetTranslationsReplaced(
      'text.alternateCompletionExpandedInfo',
      { trainingType: trainingTypeLabel, alternateTrainingType },
      true
    );
  }, [trainingTypeLabel, alternateTrainingTypeLabel]);

  const moreInfoToggleContent = useMemo(() => {
    const toggleButtonText = showAlternateCompletionDetails
      ? GetTranslation('text.showLess')
      : GetTranslation('text.moreInformation');

    const handleToggle = () => setShowAlternateCompletionDetails(prev => !prev);

    const infoTextClassName = `${styles.moreInfoText} ${showAlternateCompletionDetails ? styles.expanded : styles.collapsed}`;

    return (
      <>
        <p className={infoTextClassName}>{expandedInfoText}</p>
        <button className={styles.moreInfoButton} onClick={handleToggle} type="button">
          {toggleButtonText}
        </button>
      </>
    );
  }, [showAlternateCompletionDetails, expandedInfoText]);

  const alternateCompletionBanner = useMemo(() => {
    if (!shouldShowAlternateCompletionBanner) return null;

    const sectionIconClassName = `${styles.alternateLoSectionIcon} ${showAlternateCompletionDetails ? styles.expanded : styles.collapsed}`;

    return (
      <div className={styles.alternateLoContainer}>
        <div>
          <div className={styles.alternateLoHeader}>
            <div className={styles.alternateLoIcon}>{ALTERNATE_COMPLETION_ICON()}</div>
            <span className={styles.alternateLoTitle}>
              {GetTranslation('text.completedViaAlternate')}
            </span>
          </div>
          <div className={styles.alternateLoBody}>
            {alternateLoDescriptionContent}
            {moreInfoToggleContent}
          </div>
        </div>
        <div className={sectionIconClassName}>{ALTERNATE_LO_SECTION_ICON()}</div>
      </div>
    );
  }, [
    shouldShowAlternateCompletionBanner,
    alternateLoDescriptionContent,
    moreInfoToggleContent,
    showAlternateCompletionDetails,
  ]);
  ...
  useEffect(() => {
    if (training) {
      ...
      const urlContainsInstanceId =
        window.location.href.includes(`/${TRAINING_INSTANCE_ID_STR}`) ||
        window.location.href.includes(`/instance/`);
    ...
    return (
      <span className={styles.fileUpload} data-automationid={`${name}-certificate-upload-section`}>
        <input
          ...
          disabled={!areAllMandatoryCoursesCompleted(training, user?.account)}
          ...
          <div className={styles.pageContainer}>
            <div className={styles.left}>
              ...
              {prerequisiteLOs?.map((prerequisiteLO, index) => {
                const state = prerequisiteLO.state;
                if (state === EXPIRED) {
                  return null;
                }
            ...
            <div className={styles.right}>
              <PrimeTrainingPageMetadata
                ...
                sourceAlternateLOs={sourceAlternateLOs}
                sourceAlternateLoCount={sourceAlternateLoCount}
                loadAllSourceAlternateLOs={loadAllSourceAlternateLOs}
