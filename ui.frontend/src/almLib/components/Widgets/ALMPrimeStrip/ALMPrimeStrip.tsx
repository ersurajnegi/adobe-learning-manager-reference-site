...
import { PrimeLearningObject, PrimeTemplateConfig } from '../../../models';
import {
  GetSkillsPageLink,
  PrimeDispatchEvent,
...
import { ALMWidgetLoader } from '../../CustomPages/ALMWidgetLoader';
...
const ALMPrimeStrip = (props: any) => {
  const { widget, doRefresh = false, aoiStripCount, account, user } = props;
  ...
  const templateConfig = account
    ? (JSON.parse(account?.templatesConfig || '{}') as PrimeTemplateConfig)
    : ({} as PrimeTemplateConfig);
  const {
    showSkillsInfo = true,
    showRatingInfo = true,
  ...
  const headerLabel =
    heading?.headerAriaLabel || (heading?.name ? String(heading.name).replace(/<[^>]*>/g, '') : '');
  const headingId = useMemo(() => {
    const safe = headerLabel
      .toLowerCase()
      .replace(/[^a-z0-9_-]+/g, '-')
      .replace(/^-+|-+$/g, '');
    return `${nameId}-${safe || 'section'}`;
  }, [headerLabel]);
  const {
    feedbackTrainingId,
    trainingInstanceId,
  ...
  useEffect(() => {
    const { attributes, type, layoutAttributes } = widget;
    const numberOfCardsLoaded = items.length || 0;
    attributes!.numberOfCardsLoaded = numberOfCardsLoaded;
    const canHideList = [
    ...
    if (firstFetchDone && numberOfCardsLoaded === 0 && canHideList) {
      setState(prevState => ({ ...prevState, hideList: true }));
      attributes!.isStripHidden = true;
    }

    if (
      fetchedAll &&
      type === WidgetTypeNew.MYLEARNING &&
      layoutAttributes?.cardsToShow! >= numberOfCardsLoaded &&
      layoutAttributes?.isFullRow === false
    ) {
      widget.layoutAttributes!.overrideCardsToShow = true;
      PrimeDispatchEvent(document, PrimeEvent.ALM_RESIZE_STRIP, false);
    }
  }, [firstFetchDone, items.length, fetchedAll]);
  ...
  const onResizeInternal = useCallback(() => {
    ...
    // updateTotalItems();
  }, [numRows, itemsPerPage, containerWidth, cardContainerWidth, enableArrows]);
  useEffect(() => {
    onResizeInternal();
  }, [doRefresh]);
                    ...
                    return (
                      ...
                      >
                        <div className={styles.loadingCard}>
                          {/* Use custom card SVG for compact layout; otherwise, use ALMWidgetLoader */}
                          {getALMConfig()?._cardProperties?.cardLayoutName ===
                          COMPACT_CARD_LAYOUT ? (
  ...
  const pageTemplate = useCallback(
    ...
    [items.length, cardContainerWidth]
  );
  const getRowArrsByPageArr = useCallback(
    items => {
      ...
      for (let ii = 0; ii < pageSlicedElementsArr.length; ii++) {
        ...
        //this is to show empty card on the second row
        if (
          numRows === 2 &&
          showActionElement(widget, account) &&
          rowSlicedElementsArrPerPage.length > 0 &&
          fetchedAll &&
          (!rowSlicedElementsArrPerPage[1] || rowSlicedElementsArrPerPage[1].length < itemsPerRow)
        ) {
          rowSlicedElementsArrPerPage[1] = [
            { actionElement: true, isSecondRowAction: true } as never,
          ];
        }
  ...
  const containerHeadWithIconsHtml = () => {
    const headerAriaLabel = headerLabel;
    return (
      <>
        {heading?.name ? (
          <div className={styles.stripHeader}>
            <div className={styles.stripHeaderLeft}>
              {heading?.icon ? <div className={styles.stripHeaderIcon}>{heading?.icon}</div> : null}
              <div>
                <div className={styles.stripHeaderNameContainer}>
                  <h2 className={styles.stripContainerName}>
                    <a
                      href="#"
                  ...
                  {heading?.showNewTag ? (
                    <span className={styles.stripHeaderNewTag}>
                      {GetTranslation('alm.text.new')}
                    </span>
                  ) : null}
                </div>
                {heading?.description ? (
    ...
    return (
      <div>
        <button
          ...
          aria-label={`${headingName}, ${GetTranslation('text.leftNavigation')}`}
        ...
        <button
          ...
          aria-label={`${headingName}, ${GetTranslation('text.rightNavigation')}`}
  ...
  const getEmptyCardTemplate = useCallback(
    (isSecondRowAction?: boolean) => {
      // For single-row widgets, always show "Start Learning" (original behavior).
      // For two-row widgets, row 0 (isSecondRowAction=false) shows "Go To Catalog"
      // and row 1 (isSecondRowAction=true) shows "Start Learning".
      const effectiveIsSecondRow = numRows !== 2 ? true : isSecondRowAction;
      const { actionLink, actionHelpText, actionText, actionLinkHeading } =
        getEmptyActionCardDetails(widget, items, effectiveIsSecondRow);

      const showFullCard =
        (showSkillsInfo || showRatingInfo) && showEnrollAction && showAuthorNameInfo;

      return (
        <div className={styles.emptyCard}>
          {showFullCard && <div className={styles.emptyCardImageContainer}>{EMPTY_CARD_SVG()}</div>}
  ...
  const getItemTemplate = (widget: Widget, item: any, index: number, arrayItem: any) => {
    ...
    if (item.actionElement) {
      return getEmptyCardTemplate(item.isSecondRowAction);
    }
    return (
      <PrimeTrainingCardV2
        widget={widget}
