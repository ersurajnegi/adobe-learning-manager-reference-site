...
const PrimeCatalogSearch: React.FC<{
  query: string;
  handleSearch: (text: string, autoCorrectMode?: boolean) => void;
  getSearchSuggestions: (searhTerm: string) => any;
  updateSnippet: (checked: boolean, data: any) => void;
  autoCorrectMode: boolean;
}> = ({ query, handleSearch, getSearchSuggestions, updateSnippet, autoCorrectMode }) => {
  ...
  const searchChangedHandler = (event: any) => {
    if (event.key === 'Enter') {
      handleSearch(state, autoCorrectMode);
  ...
  const handleEnter = (key: KeyboardEvent, item: any) => {
    if (key.key === 'Enter') {
      handleClick(item);
    }
  };
  ...
  const getSuggesstionItemHtml = (item: string) => {
    return (
      <li
        data-automationid={`search_result_${item}`}
        onClick={() => {
          handleClick(item);
        }}
        title={item}
        onKeyDown={(e: any) => handleEnter(e, item)}
        className={styles.searchItemSelected}
        key={item}
        tabIndex={0}
      >
