...
const LEARNER_DESKTOP_PARAMS_MAP = {
  ...
  learnerState: {
    ...
    completedViaAlternate: 'completedViaAlternateSelected=true',
  },
  loFormat: {
  ...
  loTypes: {
    ...
    virtualCoach: 'virtualCoachSelected=true',
  },
  duration: {
...
const REACT_PARAMS_MAP = {
  ...
  learnerState: {
    ...
    completedViaAlternate: 'completedViaAlternateSelected',
  },
  loFormat: {
  ...
  loTypes: {
    ...
    virtualCoach: 'virtualCoachSelected',
  },
  duration: {
...
export function convertToLearnerDesktopParams(params?: any) {
  const url = new URL(window.location.href);
  let paramsList = url.search.substring(1).split('&');
  if (params) {
    paramsList = Object.keys(params).map(key => `${key}=${params[key]}`);
  }
        ...
        : new URLSearchParams(
            `param=${
              typeof params[key] !== 'string'
                ? params[key]
                : formatString(
                    key === SELECTED_SORT_OPTION
                      ? REACT_DESKTOP_SORT_PARAMS_MAP[sortOption]
                      : params[key]
                  )
            }`
          ).get('param');
