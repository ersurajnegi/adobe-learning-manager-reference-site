...
import { Divider, Item, Key, lightTheme, Picker, Provider } from '@adobe/react-spectrum';
...
import { getEnrollment, getLoId, getLoName } from '../../../utils/hooks';
...
const PrimeInstancePage = (props: PrimeInstancePageProps) => {
  const { user } = useUserContext() || {};
  const account = user?.account as PrimeAccount;
  const [trainingId] = useState(() => {
    if (props.trainingId) {
      return props.trainingId;
    }
    let { instancePath } = getALMConfig();
    let pathParams = getPathParams(instancePath, [TRAINING_ID_STR]);
    return customEncode(pathParams[TRAINING_ID_STR]);
  });

  const [viewType, setviewType] = useState(getInitialView(account?.viewType || LIST_VIEW));
  ...
  const [selectedLanguage, setSelectedLanguage] = useState<Key>('all');
  const [languageDropdownObject, setLanguageDropdownObject] = useState<{ [key: string]: string }>(
    {}
  );
  ...
  const { isDesktop, isMobile, isTablet } = useDeviceTypeContext();

  useEffect(() => {
    if (activeInstances && activeInstances.length) {
      const loInstanceLocales = getLoInstanceLocales(activeInstances);

      if (loInstanceLocales.size === 1 && loInstanceLocales.has(undefined)) {
        setShowLanguageDropdown(false);
      } else {
        setShowLanguageDropdown(true);
      }

      const languageDropdownObject = getLanguageDropdownObject(
        account.contentLocales,
        loInstanceLocales
      );
      setLanguageDropdownObject(languageDropdownObject);
    }
  }, [activeInstances && activeInstances.length]);

  useEffect(() => {
    if (list && list.length) {
      const tempList = filterInstanceList(list, selectedLanguage);
  ...
  const showParentBreadCrumbs = () => {
    ...
    return (
      <div className={styles.breadcrumbParent}>
        ...
        <b data-automationid={`breadcrumb-all-instances`}>
        ...
        <div className={styles.backgroundPage}>
          ...
          <section className={styles.pageContainerBackground}>
            <section className={styles.pageContainer}>
              ...
              {(isMobile || isTablet) && <Divider size="S" />}
              ...
              <div className={styles.languageFilterWrapper}>
                {showLanguageDropdown && (
                  <Picker
                    isQuiet={isMobile || isTablet ? true : false}
                    label={
                      <span className={styles.languageFilter}>
                        {GetTranslation('alm.text.language')}
                      </span>
                    }
                    labelPosition="side"
                    labelAlign="start"
                    items={Object.entries(languageDropdownObject)}
                    defaultSelectedKey="all"
                    onSelectionChange={selectedLanguage => setSelectedLanguage(selectedLanguage!)}
                    UNSAFE_style={{ alignItems: 'baseline' }}
                  >
                    {languageDropdownObj => (
                      <Item key={languageDropdownObj[0]}>{languageDropdownObj[1]}</Item>
                    )}
                  </Picker>
                )}
                {!showNoActiveInstanceMessage && (
                  <div className={styles.viewButtonBox}>
                    <button
                      className={
                        viewType === LIST_VIEW ? styles.viewButtonShaded : styles.viewButton
                      }
                    ...
                    <button
                      className={
                        viewType === TILE_VIEW ? styles.viewButtonShaded : styles.viewButton
                      }
              ...
              {isDesktop ? (
                        ...
                        <div className={styles.middleSection}>
                          ...
                          {showColumn(LOCATION) && (
                            ...
                            >
                              ...
                              <span onClick={() => applySort(LOCATION)} className={styles.sortIcon}>
                                {SORT_ORDER_SVG()}
                              </span>
                            </div>
                          )}
                        ...
                        <div className={styles.instanceList}>
                          {filteredList!.map((item: any) => (
                            <PrimeInstanceItem
                              ...
                              instanceLanguage={languageDropdownObject[item.locale]}
                              showStartDateAndInstructor={showStartDateAndInstructor ? true : false}
                              showLocation={
                                showStartDateAndInstructor ? showColumn(LOCATION) : false
                              }
                              startDate={showStartDateAndInstructor ? item.startDate : null}
                              location={showStartDateAndInstructor ? item.location : null}
                              instructorsName={
                                showStartDateAndInstructor ? item.instructorsName : null
                              }
                              seatLimit={showStartDateAndInstructor ? item.seatLimit : null}
                              seatsAvailable={
                                showStartDateAndInstructor ? item.seatsAvailable : null
                              }
                              hasCrVcModule={showStartDateAndInstructor ? hasCrVcModule : null}
                              waitlistPosition={
                                showStartDateAndInstructor ? waitlistPosition : null
                              }
                            />
                          ))}
                        </div>
                      ...
                      {filteredList!.map((item: any) => (
                        <li key={item.id} className={styles.listItem}>
                          <PrimeInstanceCard
                            ...
                            instanceLanguage={languageDropdownObject[item.locale]}
                            seatLimit={showStartDateAndInstructor ? item.seatLimit : null}
                            seatsAvailable={showStartDateAndInstructor ? item.seatsAvailable : null}
                            hasCrVcModule={showStartDateAndInstructor ? hasCrVcModule : null}
                            waitlistPosition={showStartDateAndInstructor ? waitlistPosition : null}
                          />
                        </li>
                      ))}
                    </ul>
                ...
                isListView() ? (
                  <div className={styles.instancesMobileContainer}>
                    {filteredList!.map((item: any) => (
                      <PrimeInstanceCardMobile
                        ...
                        extensionClickHandler={extensionClickHandler}
                        instanceLanguage={languageDropdownObject[item.locale]}
                        extensionLocalizedMetadata={extensionLocalizedMetadata}
                        loFormat={loFormat}
                        instanceName={instanceName}
                        languageText={languageText}
                        seatsAvailableText={seatsAvailableText}
                        showStartDateAndInstructor={showStartDateAndInstructor ? true : false}
                        showLocation={showStartDateAndInstructor ? showColumn(LOCATION) : false}
                        startDate={showStartDateAndInstructor ? item.startDate : null}
                        location={showStartDateAndInstructor ? item.location : null}
                        instructorsName={showStartDateAndInstructor ? item.instructorsName : null}
                        seatLimit={showStartDateAndInstructor ? item.seatLimit : null}
                        seatsAvailable={showStartDateAndInstructor ? item.seatsAvailable : null}
                        hasCrVcModule={showStartDateAndInstructor ? hasCrVcModule : null}
                        waitlistPosition={showStartDateAndInstructor ? waitlistPosition : null}
