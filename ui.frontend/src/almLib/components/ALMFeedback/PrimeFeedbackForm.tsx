...
const PrimeFeedbackForm: React.FC<{
  ...
  const getRadioOptions = (questionValues: any) => {
    return questionValues.map((item: any, index: number) => {
      return (
        <span
          key={`${questionItem.questionId}-${index}`}
          className={styles.feedback_l1_radio_button_container}
        >
  ...
  const getLikeabilityQuestionData = () => {
    return (
      <form>
        <div
          className={`${styles.feedback_l1_radio_buttons_group} ${styles.feedback_likert_vertical}`}
        >
