...
const getCardBackgroundStyle = (
  ...
  return training.imageUrl
    ...
    : {
        ...
        backgroundRepeat: 'no-repeat',
        backgroundSize: `calc(${isListView ? backgroundSize || '80px' : '120px'} * 9/16)`,
      };
};
...
const filterChecklistResources = (
  trainingInstance: PrimeLearningObjectInstance,
  loResourceType: string
): PrimeLearningObjectResource[] => {
  return (
    trainingInstance?.loResources?.filter(
      (loResource: PrimeLearningObjectResource) =>
        loResource.resourceType === ACTIVITY && loResource.resourceSubType === loResourceType
    ) || []
  );
};
...
const useRatingsTemplate = (
  ...
  return useMemo(() => {
    ...
    return (
      <div
        className={`${ratingsContainer} ${starContainer}`}
        title={ratingLabel}
        data-automationid={`${training?.localizedMetadata[0]?.name}-ratingContainer`}
      >
        {<Star />}
