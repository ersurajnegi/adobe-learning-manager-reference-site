...
const PrimeNoteItem: React.FC<{
  ...
  const formattedNoteMarker = (note: PrimeNote) => {
    ...
    if (contentType === CP && note.marker?.includes(NOTE_SLIDE_PREFIX_STR)) {
      const slideNumber = note.marker.split(NOTE_SLIDE_PREFIX_STR)[1];
      return GetTranslationReplaced('alm.text.slide', `${slideNumber}`);
    } else if (
      contentType === CP ||
      contentType === PR ||
      contentType === VIDEO ||
