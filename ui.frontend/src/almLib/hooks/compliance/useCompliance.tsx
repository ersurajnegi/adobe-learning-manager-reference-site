...
import { getALMConfig, isAccAltCompletionEnabled } from '../../utils/global';
...
export const useCompliance = (
  ...
  // Instance-specific loading state to prevent interference between multiple widgets
  ...
  async function fetchBatchEnrollments(category = '') {
    ...
    setIsLoading(prev => ({ ...prev, [tempCategory]: true }));

    const [enrollmentIds, idxToSet] = fetchEnrollmentIds(tempCategory);

    if (enrollmentIds.length === 0) {
      setIsLoading(prev => ({ ...prev, [tempCategory]: false }));
      return;
    }

    const { filteredEnrollmentList, filteredOutIds } = await fetchEnrollments(enrollmentIds);

    updateEnrollmentsData(tempCategory, filteredEnrollmentList, filteredOutIds, idxToSet);
    setIsLoading(prev => ({ ...prev, [tempCategory]: false }));
  }
  ...
  async function fetchEnrollments(enrollmentIds: string[]) {
    ...
    const parsedData = JsonApiParse(response);
    const { learningObjectInstanceEnrollmentList } = parsedData;

    // Filter out enrollments where learningObject.isAlternateComplete is true
    // Only filter if enableAlternateCompletion is enabled at account level
    const enableAlternateCompletion = isAccAltCompletionEnabled(account);
    const filteredEnrollmentList = enableAlternateCompletion
      ? learningObjectInstanceEnrollmentList.filter(
          (enrollment: any) => !enrollment.learningObject?.isAlternateComplete
        )
      : learningObjectInstanceEnrollmentList;

    // Get IDs of filtered out enrollments
    const filteredOutIds = enableAlternateCompletion
      ? learningObjectInstanceEnrollmentList
          .filter((enrollment: any) => enrollment.learningObject?.isAlternateComplete)
          .map((enrollment: any) => enrollment.id)
      : [];

    return { filteredEnrollmentList, filteredOutIds };
  }
  ...
  function updateEnrollmentsData(
    ...
    if (category === ALL_DEADLINES) {
      setAllEnrollmentsData(prevState => {
        // Remove filtered out IDs from enrollmentIds array
        const updatedEnrollmentIds = prevState.enrollmentIds.filter(
          id => !filteredOutIds.includes(id)
        );
        ...
        return {
          ...
          count: updatedEnrollmentIds.length,
          enrollmentList: {
      ...
      setComplianceData(prevState => {
        // Remove filtered out IDs from category's enrollmentIds array
        const updatedEnrollmentIds = prevState[category].enrollmentIds.filter(
          id => !filteredOutIds.includes(id)
        );
  ...
  // Compute total enrollment count as sum of all three category counts
  const totalEnrollmentCount =
    complianceData.OVERDUE.count + complianceData.UPCOMING.count + complianceData.ONTRACK.count;
