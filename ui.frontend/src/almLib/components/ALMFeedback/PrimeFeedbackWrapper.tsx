...
import {
  emitL1FeedbackLaunchSetup,
  emitL1FeedbackLaunched,
...
const PrimeFeedbackWrapper: React.FC<{
  ...
  const getNotificationsForLOs = async (
    ...
    // Only close immediately when API confirmed all notifications exist but all have actionTaken=true
    // (user already gave feedback). Otherwise retry - notification may be delayed after completion.
    if (loNotificationMap.current.size > 0) {
      emitL1FeedbackLaunched();
    } else if (allNotificationsReceived) {
      // API returned all notifications but all have actionTaken=true - close gracefully
      setGeneratingFeedback(false);
      emitL1FeedbackClosed([]);
      closeFeedbackPopUp();
      return;
    ...
    const interval = setInterval(async () => {
      if (numRetries.current === 2) {
        ...
        // Show "feedback taking longer" dialog - user can close to dismiss
        setGeneratingFeedback(false);
        ...
        if (allNotificationsReceived || loNotificationMap.current.size >= 10) {
          ...
          if (loNotificationMap.current.size > 0) {
            ...
            // All notifications have actionTaken=true after retries
            emitL1FeedbackClosed([]);
            closeFeedbackPopUp();
          }
          return;
  ...
  const processL1FeedbackEmailNotification = async (learningObject: PrimeLearningObject) => {
    ...
    if (emailNotificationId) {
      ...
      if (emailNotification && !emailNotification.actionTaken) {
        ...
        setLaunchFeedbackDialog(true);
        return;
      } else if (emailNotification && emailNotification.actionTaken) {
        // Notification already has actionTaken = true, re-enable navigation controls
        emitL1FeedbackClosed([]);
        closeFeedbackPopUp();
        return;
      }
  ...
  const launchL1Feedback = () => {
    if (orderedFeedbackLO.length === 0) {
      return null;
    }
    const learningObjectForFeedback = orderedFeedbackLO[0];
    ...
    return (
      <PrimeFeedbackDialog
        ...
        trainingInstance={enrolledInstance || learningObjectForFeedback.enrollment.loInstance}
  ...
  const onL1FeedbackClose = (parentLoId: string, notificationId: string) => {
    if (parentLoId === learningObject?.id && learningObject?.loType === LEARNING_PROGRAM) {
      const remaining = orderedFeedbackLO.slice(1);
      setOrderedFeedbackLO(remaining);
