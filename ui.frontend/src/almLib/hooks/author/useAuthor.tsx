...
import { PrimeLearningObject, PrimeRecommendation, PrimeUser } from '../../models/PrimeModels';
import {
  loadTrainings,
  paginateTrainings,
...
import { getItemIndexFromList } from '../../components/Widgets/ALMPrimeStrip/ALMPrimeStrip.helper';
  ...
  const addBookmarkHandler = useCallback(
    async (loId: string) => {
      try {
        ...
        if (index !== -1) {
          (list[index] as PrimeLearningObject).isBookmarked = true;
        }
  ...
  const removeBookmarkHandler = useCallback(
    async (loId: string) => {
      try {
        ...
        const index = getItemIndexFromList(list, loId, true);
        if (index !== -1) {
          (list[index] as PrimeLearningObject).isBookmarked = false;
        }
