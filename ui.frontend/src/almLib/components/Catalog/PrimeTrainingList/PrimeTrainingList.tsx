...
import { COMPLETED, JOBAID, LEARNING_PROGRAM, OTHER } from '../../../utils/constants';
...
import { storeActionInNonLoggedMode } from '../../../utils/overview';
...
const PrimeTrainingList: React.FC<{
  ...
  const completionStatusHtml = useMemo(() => {
    ...
    if (isStandardCompleted) {
      return (
        <div className={styles.completionStatus} title={GetTranslation('text.completed', true)}>
          {STANDARD_COMPLETION_ICON()}
        </div>
      );
    } else if (isAlternateCompleted) {
      return (
        <div
          className={styles.completionStatus}
          title={GetTranslation('text.completedViaAlternate', true)}
        >
          {ALTERNATE_COMPLETION_ICON()}
        </div>
      );
  ...
  const imageContainerAriaLabel = useMemo(() => {
    const isStandardCompleted = training.enrollment?.state === COMPLETED;
    const isAlternateCompleted = isAccAltCompletionEnabled(account) && training.isAlternateComplete;

    if (isStandardCompleted) {
      return GetTranslation('text.completed', true);
    } else if (isAlternateCompleted) {
      return GetTranslation('text.completedViaAlternate', true);
    }
    return undefined;
  }, [training.enrollment?.state, training.isAlternateComplete, account]);
  ...
  const showStateButton = () => {
    ...
    return (
      <button className={styles.exploreButton} onClick={cardClickHandler} title={buttonText}>
        {buttonText}
      </button>
    );
  };
  ...
  const getMetaDataHtml = () => {
    return (
      <>
        {/* Due Date starts */}
        {((isMobile && dueDate.value) || isTablet || isDesktop) && (
  ...
  }, [almAlert, enroll, handleJobAidClick, training, guest]);
  ...
  const redirectActionHandler = (event: React.MouseEvent<HTMLAnchorElement | HTMLDivElement>) => {
    ...
    if (closeModal) {
      ...
      requestAnimationFrame(() => {
        type === JOBAID ? exploreClickHandler() : cardClickHandler();
      });
    } else {
      type === JOBAID ? exploreClickHandler() : cardClickHandler();
    }
  ...
  return (
    <>
      <li className={styles.listItem}>
        ...
        {isMobile && descriptionValue && getDescription()}
        <div className={styles.skillsContainerMobile}>
          <div className={styles.loSkillsData}>
            {
              <>
                <span className={styles.skillsLabel}>
