...
export function openLink(link: string, target: string) {
  window.open(link, target);
  return;
}

export function openExtensionInSameTab(url: string, params: any = {}) {
  const appUrl = getExtensionAppUrl(url, params);
  return openLink(appUrl.href, '_top');
}

export function openExtensionInNewTab(url: string, params: any = {}) {
  const appUrl = getExtensionAppUrl(url, params);
  return openLink(appUrl.href, '_blank');
}

export function getExtensionAppUrl(url: string, params: any = {}): URL {
...
export function getExtensionWithinvocationType(
  ...
  return extensions.find((extension: PrimeExtension) => {
    return extension.invocationType === invocationType && extension.defaultScope === scope;
  });
}
...
export function getExtension(
  ...
  const filteredOverrideExtensions = overrideExtensions.filter(
    (extension: PrimeExtensionOverride) => extensionMap.get(extension.id)
  );
  ...
  const enabledExtensions = filteredOverrideExtensions.find(
    (extension: PrimeExtensionOverride) => extension.enabled === 'true'
  );
  ...
  return extensions.find(
    (extension: PrimeExtension) =>
      enabledExtensions.id === extension.id && extension.invocationType === invocationType
  );
}

export function getExtensionWithId(extensions: PrimeExtension[] = [], extensionId: string) {
  if (extensions && extensions.length === 0) {
    return;
  }
