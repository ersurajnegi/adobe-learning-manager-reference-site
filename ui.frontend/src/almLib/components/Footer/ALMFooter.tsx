...
import { injectCss } from '../../utils/widgets/utils';
// @ts-ignore
...
const ALMFooter = (props: any) => {
  ...
  const { isFooterStylingEnabled, html } = useFooter(props.footerConfig);
  ...
  return (
    <>
      {isFooterStylingEnabled ? (
        <div id="custom-footer-injections" dangerouslySetInnerHTML={{ __html: html }} />
      ) : (
        <div className={styles.header}>
          <div className={styles.footer}>
            <div className={styles.footerMenu}>
              {footerDataRows?.map((footerData, index) => (
                <Fragment key={`footer-item-${index}`}>
                  <button
