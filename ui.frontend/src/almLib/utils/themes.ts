...
export function InitThemeData(themeDataStr: string, themeDataOverrides: IThemeData): void {
  ...
  const themeCssUrl = themeData.url;
  if (themeCssUrl) {
    AddStyleSheetToHead(themeCssUrl, 'theme-css');
  }
  const tileColors =
...
function intiCssVariablesFromThems(theme: PrimeTheme) {
  ...
  setCssVariable('--prime-brand-text-color', GetTextColor(brandColor));
  setCssVariable('--prime-color-secondary', secondaryColor);
  setCssVariable('--prime-color-sidebarColor', sidebarColor);
  setCssVariable('--prime-sidebar-text-color', GetTextColor(sidebarColor));
...
export const getTileImageIndex = (id: string) => {
  ...
  const numericId = parts.length > 1 ? parts[1] : parts[0]; // Handle both formats
  return parseInt(numericId, 10) % 12 || 0;
};
