...
import { getInstanceSummary, checkIsLockedForDisplay } from '../../../utils/lo-utils';
...
const PrimeCourseItemContainer: React.FC<{
  ...
  const mappedInstanceId = courseInstanceMap[trainingId] || '';
  const trainingInstance = filterTrainingInstance(training, mappedInstanceId);
  ...
  useEffect(() => {
    ...
    let instanceName = '';
    let instanceId = '';
    let isCollapsed = training.id !== lastPlayingCourseId;
  ...
  useEffect(() => {
    if (enrolledTrainingInstance && isParentFlexLP) {
      // Initalization
